...
def fix_cefpython_api_header_file():
    ...
    if WINDOWS:
        ...
        if pragma in contents:
            ...
            # need to preserve genearted file header otherwise cython would complain
            # Cython/Utils.py#is_cython_generated_file
            contents = contents.splitlines()
            for index, content_line in enumerate(contents):
                if content_line.startswith("/* Generated by Cython "):
                    break
            contents.insert(index + 1, "%s\n\n" % pragma)
            contents = '\n'.join(contents)
